    ...
    try {
        ...
        await db.schema.createTable('user_activities')
            ...
            .addColumn('activity_points_short', 'integer', col =>
                col.notNull().defaultTo(0).check(sql`activity_points_short >= 0`))
            .addColumn('activity_points_long', 'integer', col =>
                col.notNull().defaultTo(0).check(sql`activity_points_long >= 0`))
            .addColumn('activity_points_short_ema', 'integer', col =>
                col.notNull().defaultTo(0).check(sql`activity_points_short_ema >= 0`))
            .addColumn('activity_points_short_emsd', 'integer', col =>
                col.notNull().defaultTo(0).check(sql`activity_points_short_emsd >= 0`))
