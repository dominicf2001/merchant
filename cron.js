...
async function calculateAndUpdateStocks(interval='default'){
    ...
    const activityWeight = 0.17;
    const randomWeight = 0.02;
    const netWorthWeight = 0.01;
    const priceWeight = 0.84;

    const activityDecay = (interval == '5min') ? getRandomFloat(.065, .085) : getRandomFloat(.15, .40);
    ...
    try {
        ...
        for (let latestStock of latestStocks) {
            ...
            const randomFactor = getRandomFloat(10, 100);
